...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification
{
	...
	if (!res) { NSBeep();   NSLog(@"failed to login %@ ", self.voice.errorDescription);	}
	else {
		GVoiceAllSettings *allSettings = [self.voice fetchSettings];
		NSD *ali = [allSettings propertiesPlease];
		NSLog(@"%@", ali);// subdictionaryWithKeys:[ali.allKeys arrayByRemovingObject:@"general"]]);


		id phones = [[[allSettings valueForKeyPath:@"phones"] allValues ]
											valueForKeyPath: @"formattedNumber"];
		NSLog(@"phone:  %@", phones);
		[_tv insertText:[NSS stringFromArray:phones]];

	}
	NSS *save = [AZTEMPD withPath:@"sms.plist"];
	self.smsD = [self.voice fetchSms];
	[_smsD writeToFile:save atomically:YES];
	[AZWORKSPACE openFile:save];
	[_tv insertText:[NSS stringFromArray:[[_smsD allKeys] valueForKey:@"class" ]]];// valueForKey:@"messages"]]];
...
- (void) testIsLoggedIn {
	...
	NSLog(@"We are %@", res ? @"LOGGED IN!" : @"NOT LOGGED IN :(");
